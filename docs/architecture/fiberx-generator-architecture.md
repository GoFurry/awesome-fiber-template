...
- `generator/presets/*`：preset manifests
- `generator/capabilities/*`：capability manifests
- `generator/rules/*`：replace rules 与 injection rules

它们属于生成器维护的数据声明，不是 `/v3/*` 的镜像目录。
