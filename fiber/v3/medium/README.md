...
`medium` is the balanced edition of this scaffold. It keeps the infrastructure and middleware baseline from the heavier version, but the business layer is intentionally simpler: plain `controller`, `dao`, `service`, and `models`, with routes registered directly in `url.go`.
...
This version is meant to sit between `light` and `heavy`.

- Compared with `heavy`, it removes module assembly ceremony from the business layer.
- Compared with `light`, it still keeps more infrastructure, middleware, and runtime wiring ready to use.

If you want a cleaner day-to-day service template without the heavier module pattern, this is the intended direction.
...
That is the only schema bootstrap behavior kept in `medium`.
